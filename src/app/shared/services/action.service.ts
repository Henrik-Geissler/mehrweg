...
import type { FileSaverService } from 'ngx-filesaver';
...
export class ActionService {
  constructor(
    private readonly databaseService: NgxIndexedDBService,
  ...
  public handleScan(resultString: string): Observable<number> {
    ...
    if (uppercaseResult.startsWith('MAILTO:')) {
      ...
      uppercaseResult.startsWith('BEGIN: VEVENT') ||
      uppercaseResult.startsWith('BEGIN:VEVENT')
    ) {
      ...
      uppercaseResult.startsWith('BEGIN:VCARD') ||
      uppercaseResult.startsWith('BEGIN: VCARD')
    ) {
      ...
      uppercaseResult.startsWith('HTTP://') ||
      uppercaseResult.startsWith('HTTPS://')
    ) {
      type = ActionType.URL
    ...
    return from(
      this.dbService.add(
        QR_DB_STORE,
        new QR(QrType.SCANNED, type, dataType, resultString)
      )
    )
  }
