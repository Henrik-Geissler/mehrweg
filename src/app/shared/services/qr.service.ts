...
import type { NgxIndexedDBService } from 'ngx-indexed-db';
...
import type { Contact } from '../model/qr-data/contact';
import type { Mail } from '../model/qr-data/mail';
import type { Sms } from '../model/qr-data/sms';
import type { VEvent } from '../model/qr-data/vevent';
import type { Wifi } from '../model/qr-data/wifi';
...
export class QrService {
  constructor(private readonly databaseService: NgxIndexedDBService) {}

  public loadHistory(
    groupType: QrHistoryGroupType = QrHistoryGroupType.GROUP_BY_DATE
  ): Observable<Map<string, QR[]>> {
    return from(this.dbService.getAll<QR>(QR_DB_STORE)).pipe(
  ...
  public getEssentialData(qr: QR): string {
    switch (qr.dataType) {
      case DataType.VCARD:
        const contacts = QrDataParserRegistry.getParser<Contact[]>(
          DataType.VCARD
        ).parse(qr.data)
...
return (
      ...
      case DataType.MATMSG:
        return QrDataParserRegistry.getParser<Mail>(DataType.MATMSG).parse(
          qr.data
        ).subject
      case DataType.MAILTO:
        return QrDataParserRegistry.getParser<Mail>(DataType.MAILTO).parse(
          qr.data
        ).subject
      case DataType.SMS:
        // TODO: Translate
        return (
          `Sms for: ${ 
          QrDataParserRegistry.getParser<Sms>(DataType.SMS)
            .parse(qr.data)
            .numbers.join(' - ')}`
        )
      case DataType.VEVENT:
        return QrDataParserRegistry.getParser<VEvent[]>(DataType.VEVENT).parse(
          qr.data
        )[0].summary
      case DataType.MECARD:
        const mecard = QrDataParserRegistry.getParser<Contact>(
          DataType.MECARD
        ).parse(qr.data)
        
return (
          mecard.displayName ||
          `${mecard.lastName || ''  } - ${  mecard.firstName || ''}`
        )
      case DataType.PHONE:
        return QrDataParserRegistry.getParser<string>(DataType.PHONE).parse(
          qr.data
        )
      case DataType.WIFI:
        return QrDataParserRegistry.getParser<Wifi>(DataType.WIFI).parse(
          qr.data
        ).ssid
      case DataType.TEXT:
      case DataType.URL:
  ...
  public getQrById(id: number): Observable<QR> {
    return from(this.dbService.getByID<QR>(QR_DB_STORE, Number(id)))
  }

  public updateQR(qr: QR): Observable<boolean> {
    return from(this.dbService.update<QR>(QR_DB_STORE, qr)).pipe(
      map(() => true)
    )
  }

  public saveQR(qr: QR): Observable<boolean> {
    return from(this.dbService.add<QR>(QR_DB_STORE, qr)).pipe(map(() => true))
  }

  public deleteQR(id: number): Observable<boolean> {
    return from(this.dbService.delete(QR_DB_STORE, id)).pipe(map(() => true))
  }

  public getData(qr: QR): Contact | Mail | Sms | VEvent | Wifi | string {
    switch (qr.dataType) {
      case DataType.VCARD:
        return QrDataParserRegistry.getParser<Contact[]>(DataType.VCARD).parse(
          qr.data
        )[0]
      case DataType.MATMSG:
        return QrDataParserRegistry.getParser<Mail>(DataType.MATMSG).parse(
      ...
      case DataType.MECARD:
        return QrDataParserRegistry.getParser<Contact>(DataType.MECARD).parse(
          qr.data
        )
      case DataType.PHONE:
        return QrDataParserRegistry.getParser<string>(DataType.PHONE).parse(
  ...
  private groupCodesHistory(
    groupType: QrHistoryGroupType,
    codes: QR[]
  ): Map<string, QR[]> {
    switch (groupType) {
...
return d2.getTime() - d1.getTime()
  ...
  private groupCodesHistoryByType(codes: QR[]): Map<string, QR[]> {
    const groups = codes.reduce((currentGroups, qr) => {
